...
var adminDB = {
    ...
    insertAdmin: function (adminid, username, password, callback) {   // Create/Register Admins
        ...
        dbConn.connect(function (err) {
            if (err) {
                ...
                dbConn.query(sql, [adminid, username, hashedPassword], function (err, result) {
                    dbConn.end();
                            ...
                            if (bcrypt.compareSync(password,hashedPassword)==true){
                                ...
                                conn.query(sql, [username,hashedPassword], function (err, result) {
                                    ...
                                    if (err) {
                                        ...
                                        var i;
                                        if (result.length == 1) {
                                            token = jwt.sign({ id: result[0].adminid, password: result[0].password , role:result[0].role}, config.key, {
